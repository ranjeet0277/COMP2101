...
# added all commands to variables and splitted requied command outputs
hostName=$(hostname)
#putting machine's hostname as hostName
LanAddressCMD1=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
#asssigning varibale LanAddressCMD1 for LAN address
LanAddressCMD1=$(ip a s $LanAddressCMD1 |awk '/inet /{gsub(/\/.*/,"");print $2}')
#assign variable LanHost for lan hostname
LanHostCMD1=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
LanHostCMD2=$(ip a s $LanHostCMD1 | awk '/inet /{gsub(/\/.*/,"");print $2}')
LanHost=$(getent hosts $LanHostCMD2| awk '{print $2}')
#asssigning variable for External IP and name
ExternalIP=$(curl -s icanhazip.com)
ExternalName=$(getent hosts $ExternalIP | awk '{print $2}')
# added my vm router ip to /etc/hosts and getting it's name and ip
routerIP=$(ip r s |  grep default | awk '{print $3}')
routerName=$(getent hosts $routerIP | awk '{print $2}')
cat <<EOF
Hostname        : $hostName
LAN Address     : $LanAddressCMD1
